...
template<typename T>
class distribution /*: public shared_generator*/{
public:
    distribution(){}
...
template<typename T>
class uniform_dist : public distribution<T>{
public:
...
private:
    ...
    using dist_uniform_t = std::conditional_t<
 ...
 * sigma: variance of the distr.
*/
template<typename T>
class normal_dist : public distribution<T>{
public:
...
private:
    template<typename dType>
    using dist_normal_t =  std::conditional_t<
        std::is_integral<dType>::value, 
